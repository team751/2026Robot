...
public abstract class Goal implements SimulatedArena.Simulatable {
  ...
  public static PositionChecker box(Rectangle xyBox, double minZMeters, double maxZMeters) {
    return position ->
        xyBox.contains(new Vector2(position.getX(), position.getY()))
   ...
   *
   * <p>Validates that a game piece's rotation matches an expected angle within tolerance, checking
   * both pitch and yaw. Considers both normal and flipped (180-degree rotated) orientations.
   *
  ...
  public static RotationChecker absoluteAngle(Rotation3d expectedAngle, Angle tolerance) {
    return gamePiece -> {
      ...
      double normalAngleDegrees =
          new Rotation3d(Degrees.of(0), normalDiff.getMeasureY(), normalDiff.getMeasureZ())
              .getMeasureAngle()
              .in(Units.Degrees);

      double flippedAngleDegrees =
          new Rotation3d(Degrees.of(0), flippedDiff.getMeasureY(), flippedDiff.getMeasureZ())
              .getMeasureAngle()
              .in(Units.Degrees);
   ...
   *
   * <p>Validates only the pitch (Y-axis rotation) of a game piece, ignoring yaw and roll. Useful
   * for ring-shaped game pieces where orientation around the vertical axis doesn't matter.
   *
   ...
   *
   * <p>Replaces the default rotation validation logic with a custom checker. This allows for
   * complex rotation checks beyond simple pitch/yaw tolerance, such as validating only specific
   * axes or implementing custom logic.
   *
   ...
   *
   * <p>Configures the tolerance used by the default rotation validation logic. This only works if
   * you have previously set an expected angle using {@link #setNeededAngle(Rotation3d, Angle)}.
   *
   * <p>Note: This method cannot update the tolerance without knowing the expected angle. If you
   * need to change the tolerance, use {@link #setNeededAngle(Rotation3d, Angle)} again with the new
   * tolerance.
   *
   * @param tolerance maximum allowed difference in degrees
   * @return this Goal instance for method chaining
   * @deprecated Use {@link #setNeededAngle(Rotation3d, Angle)} instead to set both angle and
   *     tolerance
   */
  @Deprecated
   ...
   *
   * <p>Replaces the default box collision with a custom shape check. This allows for cylindrical,
   * spherical, or other complex collision zones beyond axis-aligned boxes.
   *
  ...
  /**
   * Configures custom velocity requirements for scoring. This can be used to require pieces to be
   * ascending, descending, or moving within certain speed ranges.
   *
   * <p>The predicate receives the {@link GamePiece} and should return {@code true} if the velocity
   * is acceptable for scoring.
   *
   ...
   *
   * <p>Since this function is the only trigger called when a piece is scored it may handle other
   * small things outside of adding points.
   */
