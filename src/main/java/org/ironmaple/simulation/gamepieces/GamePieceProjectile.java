...
public class GamePieceProjectile implements GamePiece {
   ...
   * @param initialLaunchingVelocityMPS the horizontal component of the initial velocity in the X-Y
   *     plane, in meters per second (m/s)
   ...
   *   <li>Starts the {@link #launchedTimer}, which stores the amount of time elapsed after the game
   *       piece is launched
   * </ul>
   */
  public void launch() {
   ...
   * </ul>
   *
   * @return <code>true</code> if the game piece has flown out of the field's boundaries, otherwise
   *     <code>false</code>
   */
   ...
   *
   * <p>This method calculates the 3d velocity of the projectile using the physics formula for
   * projectile motion.
   *
   * @param t the time elapsed after the launch of the projectile, in seconds
   * @return a {@link Translation3d} object representing the calculated 3d velocity of the
   *     projectile at time <code>t
   ...
   *
   *
   * <h2>Adds a {@link GamePieceOnFieldSimulation} to a {@link SimulatedArena} to Simulate the Game
   * Piece After Touch-Ground.</h2>
   *
   ...
   *
   * <p>The added {@link GamePieceOnFieldSimulation} will always have collision space on the field,
   ...
   *
   * <p>Configures the {@link #tolerance} for determining whether the game piece has hit the target.
   * The tolerance defines how close the projectile needs to be to the target for it to be
   * considered a hit.
   *
   ...
   * href='https://github.com/Mechanical-Advantage/AdvantageScope'>Advantage Scope</a>
   *
   * @param projectileTrajectoryDisplayCallBackHitTarget the callback that will receive the list of
   *     {@link Pose3d} objects representing the projectile's trajectory, called if the projectile
   *     will hit the target on its path
   * @param projectileTrajectoryDisplayCallBackHitTargetMiss the callback that will receive the list
   ...
   *
   *
   * <h2>Registers a function that will be called whenever this game piece interacts with a goal
   * object or collides with the specified target.</h2>
   *
   * Note: Certain goals like the rebuilt hub or the reefscape processor appear to reintroduce
   * scored pieces into the field. However these pieces are actually new objects and so will not
   * save any call back functions.
   *
