...
public class SwerveModuleSimulationConfig implements Supplier<SwerveModuleSimulation> {
  ...
  public SwerveModuleSimulationConfig(
    ...
    BoundingCheck.check(
        driveFrictionVoltage.in(Volts), 0.01, 0.35, "drive friction voltage", "volts");
    BoundingCheck.check(
        steerFrictionVoltage.in(Volts), 0.01, 0.6, "steer friction voltage", "volts");
    BoundingCheck.check(wheelRadius.in(Inches), 1, 3.2, "drive wheel radius", "inches");
    BoundingCheck.check(
        steerRotationalInertia.in(KilogramSquareMeters),
        0.005,
        0.06,
        "steer rotation inertia",
        "kg * m^2");
    BoundingCheck.check(wheelsCoefficientOfFriction, 0.6, 1.9, "tire coefficient of friction", "");

    this.driveMotorConfigs =
        new SimMotorConfigs(
            driveMotorModel, driveGearRatio, KilogramSquareMeters.zero(), driveFrictionVoltage);
    this.steerMotorConfigs =
        new SimMotorConfigs(
            steerMotorModel, steerGearRatio, steerRotationalInertia, steerFrictionVoltage);
   ...
   */
  public Force getTheoreticalPropellingForcePerModule(
      Mass robotMass, int modulesCount, Current statorCurrentLimit) {
    final double
        maxThrustNewtons =
            driveMotorConfigs.calculateTorque(statorCurrentLimit).in(NewtonMeters)
                / WHEEL_RADIUS.in(Meters),
        maxGrippingNewtons =
            9.8 * robotMass.in(Kilograms) / modulesCount * WHEELS_COEFFICIENT_OF_FRICTION;
