 ...
 *
 * <p>This class simulates the <strong>HUB</strong>s on the field where <strong>FUEL</strong>s can
 * be scored. Whether it is active can be determined by using {@link
 * Arena2026Rebuilt#isActive(boolean)}
 */
public class RebuiltHub extends Goal {
	...
	public RebuiltHub(Arena2026Rebuilt arena, boolean isBlue) {
		...
		this.arena = arena;
		StructPublisher<Pose3d> HubPosePublisher =
				NetworkTableInstance.getDefault()
						.getStructTopic(
								"/SmartDashboard/MapleSim/Goals/" + (isBlue ? "BlueHub" : "RedHub"), Pose3d.struct)
						.publish();
