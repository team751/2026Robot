 ...
 *
 * <p>This class simulates a outpost on the field where fuel can be scored. It has a store of fuel
 ...
 * {@link #throwFuel(Rotation2d, Angle, LinearVelocity)} or {@link
 * Arena2026Rebuilt#outpostThrow(boolean, Rotation2d, Angle, LinearVelocity)},
 */
public class RebuiltOutpost extends Goal {
	...
	public RebuiltOutpost(Arena2026Rebuilt arena, boolean isBlue) {
		...
		StructPublisher<Pose3d> OutpostPublisher =
				NetworkTableInstance.getDefault()
						.getStructTopic(
								"/SmartDashboard/MapleSim/Goals/" + (isBlue ? "BlueOutpost" : "RedOutpost"),
								Pose3d.struct)
						.publish();

		StructPublisher<Pose3d> OutpostThrowPublisher =
				NetworkTableInstance.getDefault()
						.getStructTopic(
								"/SmartDashboard/MapleSim/Goals/"
										+ (isBlue ? "BlueOutpostThrow" : "RedOutpostThrow"),
								Pose3d.struct)
						.publish();
		StructPublisher<Pose3d> OutpostDumpPublisher =
				NetworkTableInstance.getDefault()
						.getStructTopic(
		...
		OutpostThrowPublisher.set(
				new Pose3d(isBlue ? blueLaunchPose : redLaunchPose, new Rotation3d()));
	}
				...
				if (isBlue) {
					drawList.add(
							new Pose3d(
									blueRenderPose.plus(
											new Translation3d(
													Inches.of(-6 * col), Inches.of(6 * row), Inches.of(1.3 * col))),
									new Rotation3d()));
				} else {
					drawList.add(
							new Pose3d(
